    ...
    class PageView{
        ...
        public function showArticuloProcesadores(){
            ...
            foreach($procesadores as $proce){
                echo '<article class="producto">' . '<h2>' . $proce->Marca . '<h2>' . '<h2>' . $proce->Modelo . '</h2>' .  '<h3>' . $proce->Socket . '<br>' . '<br>' . '$' . $proce->Valor . '</h3>'. '<br>' . 'ID: ' . $proce->ID_procesadores  . '<div class="button_slide slide_left"><a href="#">Añadir al carrito</a></div>' .'</article>';
            }
            echo '</section>';
        }
        ...
        public function showArticuloGraficas(){
            ...
            foreach($placas as $placa){
                echo '<article class="producto">' . $placa->Marca . '<h2>' . $placa->Modelo . '</h2>' .  '<p>' . $placa->Vram . '<br>' . $placa->Valor . '<br>' . 'ID: ' . $placa->ID_graficas . '</p>' .'</article>';
            }
            echo '</section>';
        }
        ...
        public function showArticuloRams(){
            ...
            foreach($rams as $ram){
                echo '<article class="producto">' . $ram->Marca . '<h2>' . $ram->Tamaño . '</h2>' .  '<p>' . $ram->Generacion . '<br>' . $ram->Velocidad . '<br>' . $ram->Valor . '<br>' . 'ID: ' . $ram->ID_RAM . '</p>' . '</article>';
            }
            echo '</section>';
        }
        ...
        public function showArticuloGabinetes(){
            ...
            foreach($gabinetes as $gabinete){
                echo '<article class="producto">' . $gabinete->Marca . '<h2>' . $gabinete->Modelo . '</h2>' .  '<p>' . $gabinete->Tamaño . '<br>' . $gabinete->Valor . '<br>' . 'ID: ' . $gabinete->ID_gabinete . '</p>' .'</article>';
            }
            echo '</section>';
        }
    }
